...
应用只出现在屏幕顶部菜单栏，不显示 Dock 图标。也可以将生成的 `.app` 拖到自己的“应用程序”目录。菜单栏使用 Veyra 标志，并随系统背景自动调整深浅；面板和设置保留原始米灰色标志。弹窗为 360pt 紧凑面板：高度随实际内容自动伸缩，达到所在显示器的可用高度上限后，仅中间正文滚动，应用标题和底部操作始终固定。标题栏左侧为 logo 和 Veyra，右侧为状态圆点与“本机活动”。运行任务标题与数量右侧显示任务刷新进度，标题行最右侧提供刷新按钮，按钮左侧的 info 图标悬停立即显示检查间隔、移开后隐藏；额度标题行最右侧的循环箭头按钮用于联网校准，校准进度紧跟在“额度”标题右侧。系统负责弹窗材质、圆角与阴影，滚动卡片使用语义填充；macOS 26 的固定操作按钮使用原生 Liquid Glass，旧系统使用原生 bordered 样式。外观跟随系统浅色／深色切换，并支持“减少透明度”和“增加对比度”。点击图标后，首先显示运行任务及累计、输入、输出 token；任务标题前不显示图标，点击累计用量展开缓存和推理明细。刷新保留仍存在任务的展开状态，不主动滚回顶部。向下滚动可查看全部额度窗口，底部齿轮打开设置，电源按钮退出应用。
...
卡片内以信息分组区分层次：任务左侧汇总耗时与输入／输出，右侧突出可展开的累计用量；每种额度将窗口名称和重置时间放在左侧，右侧用 56pt 圆环展示剩余比例，百分比居中；同一卡片的不同额度行之间用分割线分隔。圆环从顶部顺时针填充，剩余 ≥30% 为绿色、10%–<30% 为橙色、<10% 为红色；0% 无彩色弧线，缺失额度显示中性底环和“—”，不足 1% 显示“<1%”。任务卡片中的较长数值自动转为纵向排列，不压小文字。卡片仅使用语义填充和系统分隔色细边界。
...
- **额度**：默认读取会话 JSONL 中的 `token_count.rate_limits`，按额度桶采用记录时间最新的完整快照。任务与额度共用日志游标；冷启动补读最近 20 个非内部会话（包含归档），每个最多尾读 256 KiB。本地记录没有账号 ID，卡片内标注“本地快照”和记录时间，菜单栏用 `~` 标记；旧快照保留记录值，不推算为满额。卡片外仅显示需要处理的错误和警告；到期／缺失重置时间在对应额度行显示。不同桶各自显示记录时间，记录缺失不会冒充完整账号额度。
...
# 渲染任务家族、额度、本地来源、过期、空记录、冷却、任务刷新和圆环边界等 20 种场景
# 输出浅色／深色及高对比度的 80 张 PNG 和 layouts.json；不启动 Codex，图片使用不透明布局外观
...
新增预览场景：`local-quota`、`stale-quota`、`no-quota`、`cooldown`，用于核对本地来源、旧快照、空记录和联网冷却。

任务家族预览：`task-family` 覆盖父子同卡、多级缩进、混合状态和长进展，`family-expanded` 同时展开父任务与子任务明细，`family-context` 覆盖父任务结束、待确认或缺失时的归属展示。

额度圆环边界预览：`quota-ring-low` 包含 0%、0.4%、9.9%、10%；`quota-ring-high` 包含 29.9%、30%、100% 和缺失额度，用于核对阈值颜色、弧长和百分比格式。

任务刷新预览：`refreshing` 保留一条任务并显示任务标题与数量右侧的刷新进度，用于检查刷新按钮禁用和顶部状态行布局。
